...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      async function getCurrentUser() {
        if (user) {
          setCurrentUser(user);
          const product = await getProducts(user.uid);
          setProducts(product);
        } else {
          setCurrentUser(null);
          setProducts([]);
  ...
  async function handleGetProduct() {
    if (currentUser) {
      try {
        const product = await getProducts(currentUser.uid);
        setProducts(product);
      } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center w-full bg-[#f7fafa] px-6 py-10 lg:px-10">
      <div className="mx-auto w-[70%]">
        ...
        <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[380px_1fr]">
          ...
          >
            <h2 className="text-base font-bold text-[#172121]">Add Product</h2>
          ...
          <div className="rounded-[14px] border border-[#dce7e6] bg-white shadow-[0_2px_6px_rgba(20,40,40,0.07)]">
            ...
            {products?.length === 0 ? (
              ...
              <ul className="divide-y max-h-90 overflow-y-auto scrollbar-none divide-[#dce7e6]">
